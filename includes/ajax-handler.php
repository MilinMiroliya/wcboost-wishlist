...
class Ajax_Handler {
    ...
    public static function init() {
        ...
        // Add the new action to remove item from wishlist when added to cart
        add_action( 'woocommerce_add_to_cart', array( 'WCBoost\Wishlist\Ajax_Handler', 'remove_from_wishlist_when_added_to_cart' ), 10, 2 );
    }
    ...
    public static function add_to_wishlist() {
        ...
        // Stop if not allowing guests to create wishlists.
    ...
    /**
     * Automatically remove product from wishlist when added to cart
     */
    public static function remove_from_wishlist_when_added_to_cart( $cart_item_key, $product_id ) {
        // Get the current wishlist
        $wishlist = Helper::get_wishlist();

        // Check if the product is in the wishlist
        $item = $wishlist->get_item_by_product_id( $product_id );

        // If item exists, remove it from the wishlist
        if ( $item ) {
            $wishlist->remove_item( $item->get_key() );
        }
    }
...
Ajax_Handler::init();
add_action( 'wp_ajax_nopriv_wcboost_wishlist_add_to_wishlist', [ 'WCBoost\Wishlist\Ajax_Handler', 'add_to_wishlist' ] );
add_action( 'wp_ajax_nopriv_wcboost_wishlist_remove_wishlist_item', [ 'WCBoost\Wishlist\Ajax_Handler', 'remove_wishlist_item' ] );
add_action( 'wp_ajax_nopriv_wcboost_wishlist_get_wishlist_fragments', [ 'WCBoost\Wishlist\Ajax_Handler', 'get_refreshed_fragments' ] );
add_action( 'wp_ajax_nopriv_wcboost_wishlist_get_wishlist_items', [ 'WCBoost\Wishlist\Ajax_Handler', 'get_wishlist_items' ] );
add_action( 'wp_ajax_wcboost_wishlist_get_wishlist_items', [ 'WCBoost\Wishlist\Ajax_Handler', 'get_wishlist_items' ] );
add_action( 'wp_ajax_wcboost_wishlist_get_wishlist_fragments', [ 'WCBoost\Wishlist\Ajax_Handler', 'get_refreshed_fragments' ] );		